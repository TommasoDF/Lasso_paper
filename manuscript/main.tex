...
However, while the volume of data has surged dramatically, human capacity to manually analyze it has remained relatively constant. 
As a result, investors are increasingly turning to algorithms to detect patterns within these vast data sets.

In this paper, we propose a model that captures this dynamic. 
Even if the true underlying economic process suggests that returns are essentially unpredictable, the act of agents using statistical learning to hunt for predictability can itself generate short-lived pockets of predictability.
We show that these pockets are sparse and vary over time, echoing findings from existing literature. 
In essence, the very process of algorithmic learning brings about the temporary predictability it seeks to exploit.
