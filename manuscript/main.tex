...
\section{The model}
We study a standard \cite{Lucas1978} asset pricing model in the flavour of \cite{Adam2018}.
...
Without loss of generality in what follows we set $log(a) = s^2_d/2$. This implies that under rational expectations log-returns have mean zero and there is time $t$ information that can help forecasting returns.
...
We follow \cite{Adam2018} in making the following assumption
\begin{assumption}
...
Assumption \ref{ass:consumtion_independence} allows us to derive risk-adjusted expected gross returns are
...
Analougously to the SCEE case, we define a Cross-Moment Consistent Equilibrium (CMCE) as follows.
...
\begin{enumerate}
    ...
    \[
        r_{t+1}
        = \log(\varepsilon_{t+1})
          + \log\!\left( 1 - \kappa e^{x_t \beta} \right)
          - \log\!\left( 1 - \kappa e^{x_{t+1} \beta} \right).
    \]
...
It is clear that in a CMCE the value of $\beta$ is a solution to the equation
\begin{equation}
    T(\beta) - \beta = 0,
\end{equation}
...
and $r_{t+1}$ is given by (\ref{eq:alm_univariate}).
...
To deal with the former, we slightly change the forecast of the agent to be

\begin{equation}
\mathbb{E}\left(R_{t+1}\right) = \phi e^{w \tanh(x_t \beta/w)},
\end{equation}

where $w = -log(\kappa) - \epsilon$ for some small $\epsilon > 0$ ensures that $1 - \kappa e^{w tanh(x_{t} \beta / w)} > 0$ for all $x_t$ and $\beta$.

Now let 
\begin{equation}
    g_{\beta}(x) 
= \log\!\Bigl(1 - k\, e^{w \tanh(x \beta/w)}\Bigr),
\end{equation}

by Stein's lemma we have that
\begin{equation}
    COV(r_{t+1}, x_t) = COV(g_{\beta}(x_t), x_t) = \sigma^2_x \mathbb{E}\left[g'_{\beta}(x_t)\right].
\end{equation}

This implies that $T(\beta) = \mathbb{E}\left[g'_{\beta}(x_t)\right]$.
Altough this expression is equally intractable, we can show that  $\operatorname{sign}\left[T(\beta)\right] = - \operatorname{sign}(\beta)$ unless $\beta = 0$.
In fact, we have that

\begin{equation}
    g_{\beta}'(x)
= -\,\frac{k\,\beta\, e^{w\tanh(x \beta/w)}\, \operatorname{sech}^2(x \beta/w)}
        {1 - k e^{w\tanh(x \beta/w)}}.
\end{equation}

In the numerator, every multiplicative factor except \(\beta\) is strictly positive since
\(k>0\), \(e^{w\tanh(x \beta/w)}>0\), \(\operatorname{sech}^2(x \beta/w)>0\).
...
In particular then 
\begin{equation}
    \operatorname{sign}\left[T(\beta)\right] = \operatorname{sign}\left[ - \beta \mathbb{E}\left[-\,\frac{k\, e^{w\tanh(x \beta/w)}\, \operatorname{sech}^2(x \beta/w)}
        {1 - k e^{w\tanh(x \beta/w)}}\right]\right] = - \operatorname{sign}(\beta).
\end{equation}
...
Assuming agents use OLS to estimate the parameters in the PLM (\ref{eq:plm_univariate}), the OLS estimator $b$ can be derived from the minimization\footnote{
We use the following timing:
at time $t$ the agent forecasts $r^e_{t+1} = x_t b_{t}$ which then implies a realization for $r_t$. 
Therefore to avoid simulteneity issues, we assume that when computing the current estimate, the last available log-return observation is $r_{t-1}$.
} problem as
\begin{equation}
     b = \left(\sum^t_{i=1}  x_{i-1}^2 \right)^{-1} \sum^{t-1}_{i=1}  x_{i-1} r_i.
\end{equation}
...
Define 
\begin{equation}
M_{t-1} = \frac{1}{t-1}  \left(\sum^{t-1}_{i=1}  x_{i-1}^2 \right), \quad b_t = \frac{1}{t-1} M^{-1}_{t-1} \sum^{t-1}_{i=1}  x_{i-1} r_i.
\end{equation}

First we establish a recursion for $M_t$. Start from 

\begin{equation}
\frac{t-1}{t} M_{t-1} = \frac{1}{t}  \left(\sum^{t-1}_{i=1}  x_{i-1}^2 \right)^{-1} \implies \frac{t-1}{t} M_{t-1} + \frac{1}{t} x_{t-1}^2 = M_t.
\end{equation}
...
\begin{equation}\label{eq:b_t_evolution}
    \begin{aligned}
        b_t & = b_{t-1} + \left[(t-1) M_{t-1} \right]^{-1} \left\{ x_{t-2} r_{t-1} + \left[(t-2) M_{t-2} \right] b_{t-1} \right\} - b_{t-1} \\
        & = b_{t-1} + \left[(t-1) M_{t-1} \right]^{-1} \left\{ x_{t-2} r_{t-1} \left[(t-2) M_{t-2} - (t-1) M_{t-1}\right] b_{t-1} \right\}\\
        & = b_{t-1} + \left[(t-1) M_{t-1} \right]^{-1} \left\{ x_{t-2} r_{t-1} - x_{t-2}^2 b_{t-1} \right\}.
    \end{aligned}
\end{equation}
...
it is possible to apply results from stochastic approximation theory to study the asymptotic properties of the learning dynamics \textcolor{red}{[citations here]}.
We therefore show how to apply this framework to our model.

First we can rearrange (\ref{eq:b_t_evolution}) to get
\begin{equation}
    \begin{aligned}
        b_t & = b_{t-1} + t^{-1} M^{-1}_{t-1} \left[ x_{t-2} r_{t-1} - x_{t-2}^2 b_{t-1} \right] \\
        & + t^{-2} \frac{t}{t-1} M^{-1}_{t-1} \left[ x_{t-2} r_{t-1} - x_{t-2}^2 b_{t-2} \right].
    \end{aligned}
\end{equation}

Second, notice that $r_{t-1}$ depends on $b_{t-1}$ and $b_{t-2}$ through the ALM (\ref{eq:alm_univariate}).
We therefore define an ancillary variable $a_t = b_{t-1}$ so that
\begin{equation}
    r_{t-1} = log(\varepsilon_{t-1}) + log(1 - \kappa e^{x_{t-2} a_{t-1}}) - log(1 - \kappa e^{x_{t-1} b_{t-1}}).
\end{equation} 
...
\begin{equation}
    \begin{aligned}
        b_t = b_{t-1} + t^{-1} M^{-1}_{t-1} \left[ x_{t-2} r_{t-1} - x_{t-2}^2 b_{t-1} \right]  + t^{-2} \frac{t}{t-1} M^{-1}_{t-1} \left[ x_{t-2} r_{t-1} - x_{t-2}^2 b_{t-2} \right]\\
        a_t = a_{t-1} + t^{-1} (b_{t-1} - a_{t-1}) + t^{-2} \left(t(t-1)\right)(b_{t-1} - a_{t-1}) \\
        M_t = M_{t-1} + \frac{1}{t} \left(x_{t-1}^2 - M_{t-1} \right),
    \end{aligned}
\end{equation}


Now and we appropriately define 
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        h(\theta) = \lim_{t \to \infty} \mathbb{E} \left[\mathcal{H}(\theta, \Bar{X}_t)\right] = \begin{pmatrix}
        M^{-1} \left[ - \frac{\kappa}{1 - \kappa}a \sigma^2_x - b \sigma^2_x \right] \\
        b - a \\
        \sigma^2_x - M
    \end{pmatrix}. \\
    \end{aligned}
...
To evaluate the stability of the cross-moment consistent equilibrium we study the associated ordinary differential equation (ODE)
\begin{equation}
...
whose real part is negative, therefore the ODE is asymptotically stable for the whole range of $\kappa$ such that $\kappa > 0$. 
